...
| `worker/llm/` | 0006 llm-routing, 0023 spend controls | The ONLY place a provider is named; the four spend layers (vendor cap, per-org budget, global daily ceiling -> FakeProvider, `LLM_PROVIDER=fake`) are requirements, not options |
| `worker/` ingestion, retrieval | 0005 retrieval, 0023 worker runtime | bge-small, HNSW params, RRF; ONNX Runtime not torch; `asyncpg` + hand-written SQL, no Python ORM; tenant access only through `for_org()`, which mirrors `forOrg()` in `db/` |
| `docs/contracts/` | 0004 queue, 0023 worker runtime | Language-neutral message shapes. `triage-message.schema.json` is validated by BOTH `api/`'s and `worker/`'s test suites - the queue has no RLS, so this file is what keeps a producer bug from becoming a cross-tenant one |
