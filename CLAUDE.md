...
| `web/` pages, public form, tracking | 0021 web/db boundary, 0017 consumer creds, 0027 #27 split | `web/` reads via `@frontdesk/db` in-process, never through `api/` (0021); its DB credential follows ADR-0017's pattern (`APP_PGUSER` + `frontdesk-app-password`, `$(VAR)` expansion), never `frontdesk-password`. Form submissions enqueue through `api/` (0021). 27a = the boundary + `/t/<token>`; 27b = `/r/<slug>` + landing demo queue + Lighthouse (0027) |
