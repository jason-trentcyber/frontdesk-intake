...
| `deploy/postgres/` | 0016 postgres, 0004 queue, 0005 retrieval, 0007 tenancy | Dockerfile (pgvector base + pgmq SQL), initdb roles (`frontdesk` owner, `frontdesk_app` runtime, both `NOBYPASSRLS`), backup/restore runbook; image tag = content, never overwritten |
