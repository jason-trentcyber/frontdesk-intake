...
| `web/` pages, public form, tracking | 0021 web/db boundary, 0017 consumer creds, 0027 #27 split, 0029 Lighthouse gate | `web/` reads via `@frontdesk/db` in-process, never through `api/` (0021); its DB credential follows ADR-0017's pattern (`APP_PGUSER` + `frontdesk-app-password`, `$(VAR)` expansion), never `frontdesk-password`. The public form's Server Action calls `api/`'s intake path pod-to-pod (`API_ORIGIN`, derived from the api Service, mirrors `PUBLIC_WEB_ORIGIN`) - never by leaving the cluster through the Ingress (0021). 27a shipped the boundary + `/t/<token>`; 27b ships `/r/<slug>`, the landing demo queue, and the Lighthouse accessibility gate (0027, 0029) |
...
| `.github/` | 0008, 0011, 0014, 0029 | Provenance, review agent, gates; Dependabot exempt; deploy.yml joins the tailnet, namespace-scoped kubeconfig; `lighthouse` job is blocking, per-PR, accessibility only (0029) |
