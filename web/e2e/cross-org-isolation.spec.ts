...
import { authSessions, authUsers, createDb, forOrg, orgMembers, orgs, requests, type Db } from "@frontdesk/db";
 ...
 */
test.describe(hasEnv ? "cross-org isolation (#26)" : "cross-org isolation (#26) [skipped: DATABASE_URL/DATABASE_APP_URL not set]", () => {
  test.skip(!hasEnv);
  ...
  test.beforeAll(async () => {
    ...
    const [orgA] = await ownerDb.select({ id: orgs.id, slug: orgs.slug }).from(orgs).where(eq(orgs.slug, "bright-smile-dental"));
    const [orgB] = await ownerDb.select({ id: orgs.id, slug: orgs.slug }).from(orgs).where(eq(orgs.slug, "harbor-legal"));
    if (!orgA || !orgB) throw new Error("expected both seeded orgs (bright-smile-dental, harbor-legal) - run `pnpm seed` first");
    orgAId = orgA.id;
    orgBId = orgB.id;
    ...
    const requestAId = await forOrg(ownerDb, orgAId, async (tx, orgId) => {
      const [row] = await tx
        .insert(requests)
        .values({ orgId, source: "form", subject: orgASubject, body: "org A body", trackingToken: `tok-${randomUUID()}` })
    ...
    const requestBId = await forOrg(ownerDb, orgBId, async (tx, orgId) => {
      const [row] = await tx
        .insert(requests)
        .values({ orgId, source: "form", subject: orgBSubject, body: "org B body", trackingToken: `tok-${randomUUID()}` })
        .returning({ id: requests.id });
  ...
  test.afterAll(async () => {
    ...
    if (createdSessionTokens.length > 0) {
      await ownerDb.delete(authSessions).where(inArray(authSessions.sessionToken, createdSessionTokens));
    }
  ...
  test("removing the org_members row takes effect on the very next request - no sign-out", async () => {
    ...
    const [membership] = await ownerDb.select().from(orgMembers).where(eq(orgMembers.email, email));
