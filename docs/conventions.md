...
- **For anything RBAC-shaped, also name the client and its version.** The verb the API server authorizes is chosen by whatever writes the object, not by what the manifest looks like: Helm 4 applies hook resources with server-side apply, so "create a Job" needs `patch`, and ADR-0016's deliberate create-only grant blocked the hook entirely (ADR-0020). Corollary: pin the version of any tool whose behaviour a permission depends on — `deploy.yml` pins Helm for exactly this reason.
